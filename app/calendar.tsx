...
export default function CalendarScreen() {
    ...
    // Laddar profildata varje gång kalendern visas så den synkar med ändringar från Dashboard
    useFocusEffect(
        useCallback(() => {
            initializeData();
        }, [])
    ...
    const renderCalendarGrid = () => {
        ...
        for (let i = 0; i < firstDayOfMonth; i++) gridCells.push(<View key={`empty-${i}`} style={styles.dateCell} />);
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            gridCells.push(
                <View key={`day-${d}`} style={[styles.dateCell, isToday && styles.todayCell]}>
                    ...
                    <ThemedText style={[styles.dateText, hasEvents && { color: '#FFF', fontWeight: 'bold' }, isToday && !hasEvents && { color: '#76C800', fontWeight: 'bold' }]}>{d}</ThemedText>
        ...
        <View style={styles.mainWrapper}>
                ...
                {showDropdown && (
                    <View style={styles.dropdownContainer}>
                        {filteredAreas.length > 0 ? (
                            ...
                            <View style={styles.dropdownItem}><ThemedText style={styles.dropdownItemText}>見つかりませんでした</ThemedText></View>
                        )}
                    </View>
                )}

                <View style={styles.headerRow}>
                    <Pressable onPress={goToPreviousMonth} disabled={isFirstMonth} style={[styles.arrowButton, isFirstMonth && { opacity: 0.2 }]}><Ionicons name="chevron-back" size={28} color="#333" /></Pressable>
                    ...
                    <Pressable onPress={goToNextMonth} disabled={isLastMonth} style={[styles.arrowButton, isLastMonth && { opacity: 0.2 }]}><Ionicons name="chevron-forward" size={28} color="#333" /></Pressable>
                </View>

                <View style={styles.calendarCard}>
                    <View style={styles.weekHeaderRow}>
                        {['日', '月', '火', '水', '木', '金', '土'].map((w, index) => (
                            <ThemedText key={w} style={[styles.weekHeaderText, index === 0 && { color: '#EF4444' }, index === 6 && { color: '#3B82F6' }]}>{w}</ThemedText>
                        ))}
                    </View>
                    {loading ? <ActivityIndicator size="large" color="#76C800" style={{ marginVertical: 40 }} /> : <View style={styles.gridContainer}>{renderCalendarGrid()}</View>}
                </View>

                <View style={styles.legendContainer}>
                    <ThemedText style={styles.legendSectionTitle}>資源とごみの収集区分</ThemedText>
                    <View style={styles.legendGrid}>
                        <View style={styles.legendItem}><View style={[styles.legendColorBox, { backgroundColor: '#DC2626' }]}><Ionicons name="flame-outline" size={12} color="#FFF" /></View><ThemedText style={styles.legendLabelText}>可燃ごみ</ThemedText></View>
                        <View style={styles.legendItem}><View style={[styles.legendColorBox, { backgroundColor: '#2563EB' }]}><MaterialCommunityIcons name="fire-off" size={14} color="#FFF" /></View><ThemedText style={styles.legendLabelText}>不燃ごみ</ThemedText></View>
                        <View style={styles.legendItem}><View style={[styles.legendColorBox, { backgroundColor: '#16A34A' }]}><Ionicons name="newspaper-outline" size={12} color="#FFF" /></View><ThemedText style={styles.legendLabelText}>資源</ThemedText></View>
                        <View style={styles.legendItem}><View style={[styles.legendColorBox, { backgroundColor: '#EAB308' }]}><Ionicons name="cube-outline" size={12} color="#FFF" /></View><ThemedText style={styles.legendLabelText}>プラ容器包装</ThemedText></View>
                    </View>
                </View>
            </ScrollView>
            ...
            <View style={styles.tabBarContainer}>
                ...
                <View style={styles.tabBarContent}>
                    ...
                    <Pressable style={styles.tabItemBottom} onPress={() => router.push('/calendar')}><FontAwesome5 name="calendar-alt" size={22} color="#5B9E00" /><ThemedText style={[styles.tabLabelBottom, styles.tabLabelBottomActive]}>ゴミカレンダー</ThemedText></Pressable>
                    ...
                    <Pressable style={styles.tabItemBottom} onPress={() => router.push('/reuse')}><Ionicons name="refresh-circle-outline" size={26} color="#555" /><ThemedText style={styles.tabLabelBottom}>リユース</ThemedText></Pressable>
...
// ⚠️ KLISTRA IN DINA BEFINTLIGA STYLES HÄR! ⚠️

// ⚠️ GLÖM INTE ATT BEHÅLLA DINA STYLES HÄR NEDANFÖR! ⚠️
// klistra in din befintliga const styles = StyleSheet.create({ ... }) här.
